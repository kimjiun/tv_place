...
router.post('/info', function(req, res, next) {
  ...
  var sql_insert = 
    "insert into tvinfo_info (member_seq, name, tel, address, latitude, longitude, description) " +
    "values(?, ?, ?, ?, ?, ?, ?); ";
...
router.post('/info/image', function (req, res) {
  ...
  form.parse(req, function(err, fields, files) {
    var sql_insert = "insert into tvinfo_info_image (info_seq, filename, image_memo) values (?, ?, ?);";
...
router.get('/info/:seq', function(req, res, next) {
  ...
  var sql = 
    ...
    "  if( exists(select * from tvinfo_keep where member_seq = ? and a.seq = info_seq), 'true', 'false') as is_keep, " +
    "  (select filename from tvinfo_info_image where info_seq = a.seq order by seq limit 1) as image_filename, " + 
    " (select round(AVG(rate)) from tvinfo_rate where item_seq = a.seq) as rate_avg " +           
    "from tvinfo_info as a " +
    "where seq = ? ; ";
  console.log("sql : " + sql);
    
  db.get().query(sql, [user_latitude, user_longitude, user_latitude, member_seq, seq], function (err, rows) {
      if (err) {
...
router.get('/search', function(req, res, next) {
  ...
  var sql = 
    ...
    "  (select filename from tvinfo_info_image where info_seq = a.seq) as image_filename " +
    "from tvinfo_info as a " +
    "where " + search_type +" like '%" + search_content + "%' " +
